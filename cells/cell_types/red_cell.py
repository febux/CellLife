...
from cells.genome.red_cell__genome import RedCellGenome
...
class RedCell(Cell):
    ...
    def try_cell_mutation(cls, x: int, y: int):
        ...
        return random.choices([cls(x, y), CellType.PurpleCell.class_(x, y)], weights=(100, 4))[0]
    ...
    def recalculate_cell_energy(self, cells: List[List[TCell]]) -> TCell:
        ...
        if self.energy_capacity <= 0:
            return CellType.DeadRedCell.class_(self.x, self.y)
        else:
